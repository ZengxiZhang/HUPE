...
class FlowUpsamplerNet(nn.Module):
    def __init__(self, image_shape, hidden_channels, K, L=None,
        ...
        super().__init__()
        self.hr_size = opt['datasets']['train']['crop_size']
        ...
        self.H = H
        self.W = W
        self.scaleH = opt['datasets']['train']['crop_size'] / H
        self.scaleW = opt['datasets']['train']['crop_size'] / W
    ...
    def forward(self, gt=None, rrdbResults=None, z=None, epses=None, logdet=0., reverse=False, eps_std=None,
                y_onehot=None,return_middleflow=False):

            epses_copy = [eps for eps in epses] if isinstance(epses, list) else epses
            sr = self.decode(rrdbResults, z, eps_std, epses=epses_copy, logdet=logdet, y_onehot=y_onehot,return_middleflow=return_middleflow)
    ...
    def encode(self, gt, rrdbResults, logdet=0.0, epses=None, y_onehot=None):
        ...
        for level in range(1, L + 1):#level=1,2,3 maxpooling 3次
            bypasses[level] = torch.nn.functional.interpolate(gt, scale_factor=2 ** -level, mode='bilinear',
                                                              align_corners=False)
        indexx=0
        t=[]
        for layer, shape in zip(self.layers, self.output_shapes):#21
            size = shape[2]
            level = int(np.log(self.hr_size / size) / np.log(2))#7个1 7个2 7个3
            ...
            indexx+=1
            #每个阶段1个else，6个flowstep, 没有split2d
            if isinstance(layer, FlowStep):#每个阶段的第3到第6会将会使用rrdbResults
                fl_fea, logdet, scale = layer(fl_fea, logdet, reverse = reverse, rrdbResults = level_conditionals[level])
                if scale is not None:
                    t.append(scale)
            elif isinstance(layer, Split2d):# 没有用过
                fl_fea, logdet= self.forward_split2d(epses, fl_fea, layer, logdet, reverse, level_conditionals[level],
                                                      y_onehot=y_onehot)
            else:# squeeze 每个大阶段的第一层使用
    ...
    def decode(self, rrdbResults, z, eps_std=None, epses=None, logdet=0.0, y_onehot=None,return_middleflow=False):
        ...
        for layer, shape in zip(reversed(self.layers), reversed(self.output_shapes)):
            ...
            if isinstance(layer, Split2d):
                ...
                fl_fea, logdet,scale = layer(fl_fea, logdet=logdet, reverse=True, rrdbResults=level_conditionals[level])
                if scale is not None:
                    t.append(scale)
            else:
                
                fl_fea, logdet = layer(fl_fea, logdet=logdet, reverse=True)
                if return_middleflow:
                    flows.append(fl_fea)
        sr = fl_fea
        assert sr.shape[1] == 3
        return sr
...
def get_position_name(H, scale, opt):
    downscale_factor = opt['datasets']['train']['crop_size'] // H
