...
class _ActNorm(nn.Module):
    ...
    def forward(self, input, logdet=None, reverse=False, offset_mask=None, logs_offset=None, bias_offset=None):
        ...
        if not self.inited: # not self.inited == True
            self.initialize_parameters(input)
        self._check_input_dim(input)

        if offset_mask is not None: # default false
        ...
        if not reverse:
            ...
            input = self._center(input, reverse, bias_offset)# + self.bias 有一种搞到均值为0的意思
            # print(torch.mean(input))
            # print(torch.max(input))
            # print(torch.min(input))
            # print(logdet)
            input, logdet = self._scale(input, logdet, reverse, logs_offset)#* torch.exp(logs)
            # print(torch.mean(input))
            # print(torch.max(input))
            # print(torch.min(input))
            # print(logdet)
